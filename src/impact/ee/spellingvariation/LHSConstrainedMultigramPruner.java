...
public class LHSConstrainedMultigramPruner implements MultigramPruner
{
  ...
  public String[] extraGraphemes =
  {
	...
	java.util.Map<String,String>  allowedLHSMap = new HashMap<String,String>();
  public LHSConstrainedMultigramPruner(String filename) // patternsets/frenchG2P.graphemes
  {
  	//java.io.InputStream
  	try
  	{
  		java.io.BufferedReader r = new java.io.BufferedReader(impact.ee.util.Resource.openResourceFile(filename));
  		String s;
  		while ( (s=r.readLine()) != null)
  		{
  			allowedLHSMap.put(s,s);
  		}
  	} catch (Exception e)
  	{
  		e.printStackTrace();
  ...
  public boolean isOK(JointMultigram m) // TODO: lhs all vowels <-> rhs all vowels
  {
    ...
    if (m.isSingleton()) 
    	return true;
    return allowedLHSMap.containsKey(m.lhs);
  }
/*
