...
import nl.namescape.util.XML.NodeAction;
...
public class TEITokenStream implements TokenWindow, Corpus
{
	...
	private boolean breakForbidden(Element t)
	{
		...
		for (Node n:a)
		{
			if (n.getNodeType() == Node.ELEMENT_NODE)
				if (nl.namescape.tei.TEITagClasses.noSentenceBreakIn((Element) n))
				{
	...
	{
		List<Element> words = nl.namescape.tei.TEITagClasses.getTokenElements((Element) startNode);
		if (words.size() == 0)
			return true;
