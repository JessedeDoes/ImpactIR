...
public class NEREvaluation 
{
	...
	public void evaluate(NETaggedDocument truth, NETaggedDocument tagged)
	{
		...
		for (Entity e: trueEntities)
		{
			...
			if (e1 != null)
			{
				if (verbose) nl.openconvert.log.ConverterLog.defaultLog.println("Found " + e1);
			...
			{
				if (verbose) nl.openconvert.log.ConverterLog.defaultLog.println("Missed: " + e +  " at " + location);
		...
		for (Entity e: taggedEntities)
		{
			...
			{
				if (verbose) nl.openconvert.log.ConverterLog.defaultLog.println("False positive: " + e +  " at " + location);
	...
	public void printResults() 
	{
		...
		for (String t: types)
		{
			...
			nl.openconvert.log.ConverterLog.defaultLog.println(t + ": precision "  + precision + ", recall " + recall + " f1: " + f1);
		}
		double classificationAccuracy = truePositiveCounter.get(ALL) / (double) truePositiveCounter.get(ANY);
		nl.openconvert.log.ConverterLog.defaultLog.println("Classification accuracy:" + classificationAccuracy);
	}
