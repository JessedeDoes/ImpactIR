...
public class SVMLightModel implements Serializable {
	...
	protected Map<Integer,Double> m_linWeightsMap = new HashMap<Integer,Double>();
	protected int m_linWeightsLength=0;
	...
	public void compressLinear()
	{
		if (m_docs != null)
		{
			addWeightVectorToLinearModel();
			m_docs = null;
			for (int i=0; i < m_linWeights.length; i++)
			{
				if (m_linWeights[i] != 0)
					m_linWeightsMap.put(i,m_linWeights[i]);
			}
			this.m_linWeightsLength = m_linWeights.length;
			this.m_linWeights = null;
		}
	}

	private double getLinWeight(int i)
	{
		Double d = m_linWeightsMap.get(i); 
		if (d != null)
			return d;
		return 0;
	}
	...
	public double classify(FeatureVector v) {
		...
		if (m_kType == 0) {
			...
			{
				int dim = v.m_dims[i]; // aanpassing: check bounds m_linWeights (Jesse)
				if (dim < m_linWeightsLength)
					delta += v.m_factor * getLinWeight(v.m_dims[i]) * v.m_vals[i];
			}
		} else {
			for (int i = 0; i < m_docs.length; i++) {
