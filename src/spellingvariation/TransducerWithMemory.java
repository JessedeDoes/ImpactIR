...
public class TransducerWithMemory implements java.io.Serializable
{
	...
	static class ScoredState extends util.ShortestPath.BasicState
	{
		History.State state;
		double score = 0;
		...
		static class Link extends util.ShortestPath.BasicEdge
		{
		...
		public void relaxEdges(util.ShortestPath sp, util.ShortestPath.MatchState source)
		{
	...
	protected void printBestAlignment(SegmentationGraph g)
	{
		...
		{
			Vector<Vector<ScoredState>> alphas = forwardEvaluation(g);
			util.ShortestPath<ScoredState, ScoredState.Link> sp = new util.ShortestPath<ScoredState, ScoredState.Link>();
