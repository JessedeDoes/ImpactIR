...
public class Trie implements java.io.Serializable
{
		...
		try
		{
			...
			while((word = wordlist.readLine())  != null)
			{
				if (addWordBoundaries)
					word = spellingvariation.Alphabet.initialBoundaryString + word + spellingvariation.Alphabet.finalBoundaryString;
				if (nWords % 10000 == 0)
					System.err.printf("%s\n",word);
				if (assumeOrdered)
	...
	public  class TrieNode implements Serializable
	{
		...
		TrieNode putWord(int [] w)
		{
			return putWord(w, 0);
		}
		
		TrieNode putWord(int [] w, Object data)
		{
